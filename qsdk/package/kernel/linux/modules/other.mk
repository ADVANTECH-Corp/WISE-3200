...
define KernelPackage/tpm-spi-tis
  TITLE:= TPM 1.2 or TPM 2.0 interface spi driver
  SUBMENU:=$(OTHER_MENU)
  KCONFIG:=CONFIG_TCG_TIS_SPI
  FILES:=$(LINUX_DIR)/drivers/char/tpm/tpm_spi_tis.ko
  AUTOLOAD:= $(call AutoProbe, tpm-spi-tis)
  DEPENDS:=+kmod-tpm kmod-spi-dev
endef

define KernelPackage/tpm-spi-tis/description
  TPM security chip which is connected to a regular,
  non-tcg SPI master that is compliant with the
  TCG TIS 1.3 TPM specification (TPM1.2) or the TCG PTP FIFO
  specification (TPM2.0).
endef

$(eval $(call KernelPackage,tpm-spi-tis))
