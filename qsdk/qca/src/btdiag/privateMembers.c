...
void DiagPrintToStream(BYTE response[], int Length, bool check)
{
    ...
    static BYTE modifiedResponseMsg[TCP_BUF_LENGTH * 2];
    ...
    static BYTE messageToSend[TCP_BUF_LENGTH* 2];   //modifiedResponseMsg.Length = responseLength +2;
	...
	else
	{
		...
		dump_data(NULL, messageToSend , messageToSend_Length);
...
int processDiagPacket(BYTE inputDIAGMsg[])
{
    ...
    switch (inputDIAGMsg[0])
    {
        ...
        case 0x4B:  //(byte)CmdCode.subSystemDispatch:
            printf("\nSubsystemDispatch received msg with type 0x%02x\n", inputDIAGMsg[0]);
		...
		int payloadIndex = 10;
                ...
                /* Send the HCI command packet to UART/USB for transmission */
                int ret = 0;

		if (btDevFd < 0) {
			printf("BT device(%d) is invalid.\n", btDevFd);
			return 1;
		}
		...
		switch (connectionTypeOptions)
		{
			    ...
			    ret = write(btDevFd, msgToDUT, lengthOfPayload);
			    ...
			    break;
		    case USB:
			    ret = usb_send_hci_cmd(btDevFd, msgToDUT, lengthOfPayload);
			    if (ret != 0) {
				    fprintf(stderr, "%s: Send failed with ret value: %d\n", __FUNCTION__, ret);
				    return 0;
			    }
        ...
        case 0x7B:  //diagProtLoopback:
			...
			{
				printf("<- Send to client(%d):\n", client_sock);
				dump_data(NULL, inputDIAGMsg, indexOf7E + 1);
