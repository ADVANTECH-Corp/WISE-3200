 ...
 *
 */
int usb_send_hci_cmd(int dd, unsigned char *rawbuf, int rawdatalen)
{
	...
	printf("Send to hci device(%d):\n", dd);
	dump_data(NULL, buf, len);

	if (hci_dev_send_cmd(dd, ogf, ocf, len, buf) < 0) {
